...
func TestParsePacketBackPatchesPolyCount(t *testing.T) {
	...
	err := p.OnDemoClassInfo(&msgs2.CDemoClassInfo{
		Classes: []*msgs2.CDemoClassInfoClassT{
...
func polyFSVBytes(t *testing.T) []byte {
	...
	field := func(name, vartype, ser string, polyNames ...string) *msgs2.ProtoFlattenedSerializerFieldT {
		x := &msgs2.ProtoFlattenedSerializerFieldT{
		...
		for _, pn := range polyNames {
			x.PolymorphicTypes = append(x.PolymorphicTypes, &msgs2.ProtoFlattenedSerializerFieldTPolymorphicFieldT{
