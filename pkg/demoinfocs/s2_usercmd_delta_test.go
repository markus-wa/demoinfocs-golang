...
func subtick(button uint64) *msgs2.CSubtickMoveStep {
	return &msgs2.CSubtickMoveStep{Button: proto64(button), Pressed: protoBool(true)}
}
...
func TestApplyUserCmdDelta_RepeatedIndexPatch(t *testing.T) {
	m := &msgs2.CSGOUserCmdPB{Base: &msgs2.CBaseUserCmdPB{
		SubtickMoves: []*msgs2.CSubtickMoveStep{subtick(1), subtick(2)},
	}}
...
func TestApplyUserCmdDelta_RepeatedIndexZeroAndTruncate(t *testing.T) {
	m := &msgs2.CSGOUserCmdPB{Base: &msgs2.CBaseUserCmdPB{
		SubtickMoves: []*msgs2.CSubtickMoveStep{subtick(1), subtick(2), subtick(3)},
	}}
...
func TestApplyUserCmdDelta_RepeatedInputHistory(t *testing.T) {
	m := &msgs2.CSGOUserCmdPB{InputHistory: []*msgs2.CSGOInputHistoryEntryPB{
...
func TestServerUserCmdDeltaDataRoundTrip(t *testing.T) {
	want := &msgs2.CMsgServerUserCmd{DeltaData: []byte{0x12, 0x01, 0x00}}
	encoded, err := proto.Marshal(want)
	require.NoError(t, err)

	got := &msgs2.CMsgServerUserCmd{}
